  ...
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>预计使用时间</div>
          <van-field
              v-model="outWorkDate"
              name="openCardDate"
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>物品数量</div>
          <van-field
              v-model="goodsNum"
              name="goodsNum"
              placeholder="请输入物品数量"
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>用途</div>
          <van-field
              v-model="goodsWay"
              name="goodsWay"
              placeholder="请输入用途"
...
import {createShopApply,} from "../../../../services/apply";
...
import SelectCopyPeoplePicker from '../../../../components/selectComponent/selectCopyPeoplePicker/index.vue' // 引入选人组件
...
const selectkey = ref();
const loading=ref(false); //loading控件
let selectPersonList= []; //用于存放选择的审批人数组
const pattern = /^[0-9]+(\.[0-9]{1,2}|)$/; // 保留两位小数的金额校验
const selectCopyPersonList=ref([]); //
...
const goodsName=ref(''); //地点
const goodsNum=ref(''); // 会议人员
const goodsWay=ref('') // 会议内容
const goodsPrice=ref('') // 预算金额
// 选择时间
...
const onSubmit = async (values: any) => {
  ...
  const shopApplyDetail: any = {
    ...
    "needTime":`${startTime.value} 至 ${endTime.value}`,
    "rangeTime": [`${startTime.value}`, `${endTime.value}`],
    "goodsName":goodsName.value,
    "goodsWay":goodsWay.value,
  ...
  const rowShowJson: any = [
    ...
    {'label': '物品名称', 'value': goodsName.value, 'isImportant': '1'},
    {'label': '物品数量', 'value': goodsNum.value, 'isImportant': '1'},
    {'label': '物品用途', 'value': goodsWay.value, 'isImportant': '1'},
    {'label': '预算金额', 'value': goodsPrice.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
    ...
    "timeStatus": false,
    "approveType": "物品采购申请单",
    "shopApplyDetail": shopApplyDetail,
  ...
  loading.value=true;
  let result: any = await createShopApply(postData);
