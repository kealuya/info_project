  ...
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span> 领用用途</div>
          <van-field
        ...
        <div class="mt10">
         <div v-for="(item,index) in goods">
           <van-row justify="space-between" class=" mt10">
             <van-col>  <span class="ml5 f14" style="color:#999999">物品明细（{{index+1}}）</span></van-col>
             <van-col> <span v-if="index!=0"  class="f12  mr5" style="color:#999999" @click="delGoods(index)">删除</span>  </van-col>
           </van-row>
           <div class="formCard">
             <div class="pl5 f16"><span class="require">*</span>物品名称</div>
             <van-field
             ...
             />
             <div class="pl5 f16"><span class="require">*</span>数量</div>
             <van-field
                 v-model="item.number"
                 name="approveReason"
                 placeholder="请输入数量"
          ...
          <van-cell style="padding: 0px 15px;">
            <div style="border-top: #f3f3f3 0.5px solid" class="pt5">
              <van-row justify="center" align="center" class="mt10 pb10" @click="addGoods">
                <van-col> <van-icon name="plus" color="#0088ff" /><span style="color: #0088ff">添加物品明细</span> </van-col>
              </van-row>
            </div>
          </van-cell>
        </div>
...
import {createArticleRequisition,} from "../../../../services/apply";
...
const goods=ref([]); //物品明细
const useClassification=ref(''); //领用类型
const toUserList = ref([]); // 获取到的审批人
const ApproveUserList = ref([]); // 获取到的审批人
const ccUser = ref(''); // 获取到的抄送人
const loading=ref(false); // loading 组件
let userInfoData: any = inject("userInfo"); //获取审批数组
const selectFlowOver =ref();
const selectkey = ref();
let selectPersonList= []; // 用于存放审批人
const selectCopyPersonList=ref([]); // 用于存放抄送人
...
const onConfirm = (value: any) => {
  useClassification.value = value.selectedValues[0];
...
const onSubmit = async (values: any) => {
  concatList();
  const articleRequisitionDetail: any = {
    ...
    "deptId": userInfoData.userInfo.deptId,
    "flowId": flowId.value,
    "articleRequisition": JSON.stringify(goods.value),
    "useClassification": useClassification.value,
    "purpose": approveReason.value,
    "goods": goods.value,
  };
  const rowShowJson: any = [
    ...
    {'label': '领用用途', 'value': useClassification.value, 'isImportant': '1'},
  ];
  let title='';
  goods.value.forEach((e:any)=>{
   return  title=title+`${e.goodsName}（${e.number}个）,`;
  })
  rowShowJson.push({'label': '领用物品', 'value': title.substring(0, title.length - 1), 'isImportant': '1'},);
  const postData: any = {
    'approveNo':ywNo.value,
    ...
    "timeStatus": false,
    "approveType": "物品领用",
    "articleRequisitionDetail": articleRequisitionDetail,
  ...
  loading.value=true;
  let result: any = await createArticleRequisition(postData);
 ...
 // 初始化物品数组
const addGoods=()=>{
  const goodData:any|never={
    'goodsName':'',
    'number':"",
  }
  goods.value.push(goodData as never);
}
