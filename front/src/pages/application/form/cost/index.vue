   ...
   <nav-bar title="费用申请" />
    <div>
      <van-form @submit="onSubmit">
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>费用金额</div>
          <van-field
              v-model="goodsPrice"
              name="goodsPrice"
              placeholder="请输入预算金额"
...
import {createDailyReimbursemen} from "../../../../services/apply";
...
let minDate = new Date(nowDate.getFullYear(), nowDate.getMonth()-6, nowDate.getDay()); /// 获取六个月前的今天
...
const selectkey = ref();
let selectPersonList= []; // 用于获取选择的审批人
let selectCopyPersonList=ref([]); // 用于获取选择的抄送人
const customFieldName = {
  text: 'flow_name',
  value: 'flow_id',
};
const goodsPrice=ref([]); //金额
...
const onSubmit = async (values: any) => {
  ...
  const dailyReimbursementDetail: any = {
  ...
  const rowShowJson: any = [
    ...
    {'label': '费用金额', 'value': goodsPrice.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
    ...
    "timeStatus": false,
    "approveType": "费用申请",
    "dailyReimbursementDetail": dailyReimbursementDetail,
  ...
  loading.value=true;
  let result: any = await createDailyReimbursemen(postData);
