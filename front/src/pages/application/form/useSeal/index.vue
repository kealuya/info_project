  ...
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span> 印信种类</div>
          <van-field
              v-model="sealType"
              name="sealType"
              placeholder="请选择印信种类"
              ...
              readonly
              :rules="[{ required: true, message: '请选择印信种类' }]"
          />
...
import {createSealApply} from "../../../../services/apply";
...
const showTypePicker = ref(false); // 控制请假类型
...
const selectkey = ref();
let selectPersonList= []; //存放审批人的数组
const selectCopyPersonList =ref([]); // 存放抄送人的数组
...
// 印信种类
const onConfirmType = (value: any) => {
  sealType.value = value.selectedValues[0];
...
const onSubmit = async (values: any) => {
  ...
  const sealDetail: any = {
    ...
    "deptId": userInfoData.userInfo.deptId,
    "flowId": flowId.value,
    "useStyle": useStyle.value,
    "sealType": sealType.value,
    "userWay":  approveReason.value,
  };
  const rowShowJson: any = [
    ...
    {'label': '印信种类', 'value': sealType.value, 'isImportant': '1'},
    {'label': '使用方式', 'value': useStyle.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
  ...
  loading.value=true;
  let result: any = await createSealApply(postData);
