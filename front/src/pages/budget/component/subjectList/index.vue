<template>
  <van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="min-height: 80vh">
    ...
    <div v-if="loading==false && list.length==0" style="height: 500px">
      ...
      <div style="color: rgba(23,26,29,0.40);text-align: center">暂无预算管理</div>
    </div>
    <van-list
        v-model="loading"
        v-else
...
import {getSubjectList} from "../../../../services/budget";
...
const getBudget=async(payload:any,code:any)=>{
  let result: any = await getSubjectList(payload);
  if(code=='refresh'){
    list.value = [];
  }
