  ...
  if (obsResult.length !== 0) {
    ...
    if (result.success !== false) {
      ...
      if (okData.data && okData.data[0].code === 'normal'){
        ...
        Notify({type: 'warning', message: '图片未识别出结果，请尝试手动填写！'});
      }
    } else {
      loading.value = false;
      Notify({type: 'danger', message: '请求超时，请您稍后重试'});
    }
...
const linkToWx = async () => {
  Notify({type: 'warning', message: '暂未开放', duration: 1500,});
};
...
const messageUrl = 'http://192.168.120.220:10012/wsyyApp/#/pages1/page/webAddress/webAddress?'; //短信导入url
const emailUrl = 'http://192.168.120.220:10012/wsyyApp/#/pages1/page/emailImportImap/emailImport?'; //邮箱导入url
const couldUrl = 'http://192.168.120.220:10012/wsyyApp/#/pages1/page/miniWxInvoices/miniWxInvoices?'; //云票据箱url
const rpAUrl = 'http://192.168.120.220:10012/wsyyApp/#/pages1/page/emailImportSecond/emailImportSecond?'; //邮箱RPA
let url = '';
...
function accessTest(url: any) {
  ...
  link.onerror = function () {
    Notify({type: 'danger', message: '唤醒链接超时，请您稍后重试'});
  }
