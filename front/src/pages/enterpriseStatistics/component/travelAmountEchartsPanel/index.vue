<template>
  <div class="m10">
    ...
    <div v-if="countCorprateNum.moneyTotal!==0">
      ...
      <div>
        <div class="numCard">
          <van-row justify="space-between" align="center">
            ...
            <van-col>
              <span class="f14 bold mr5">{{ countCorprateNum.moneyTotal }}元</span>
            </van-col>
          </van-row>
        </div>
        <div class="numCard">
              ...
              <van-row align="center">
                ...
                <div>
                  ...
                  <div class="f12 ml5 graytxt">
                    在时间期限内该方式使用总数占比{{ numPercentage(countCorprateNum.flightMoney).toFixed(2) }}%
                  </div>
                </div>
              </van-row>
            </van-col>
            <van-col>
              <span class="f14 bold mr5">{{ countCorprateNum.flightMoney }}元</span>
            </van-col>
          </van-row>
        </div>
        <div class="numCard">
            ...
            <van-col>
              <van-row align="center">
                ...
                <div>
                  ...
                  <div class="f12 ml5 graytxt">
                    在时间期限内该方式使用总数占比{{ numPercentage(countCorprateNum.trainMoney).toFixed(2) }}%
                  </div>
                </div>
              </van-row>
            </van-col>
            <van-col>
              <span class="f14 bold mr5">{{ countCorprateNum.trainMoney }}元</span>
            </van-col>
          </van-row>
        </div>
        <div class="numCard">
            ...
            <van-col>
              <van-row align="center">
                ...
                <div>
                  ...
                  <div class="f12 ml5 graytxt">
                    在时间期限内该方式使用总数占比{{ numPercentage(countCorprateNum.hotelMoney).toFixed(2) }}%
                  </div>
                </div>
              </van-row>
            </van-col>
            <van-col>
              <span class="f14 bold mr5">{{ countCorprateNum.hotelMoney }}元</span>
            </van-col>
          </van-row>
        </div>
      </div>
...
const getCountCorporate = async () => {
  ...
  // 根据判断的数值追加到饼图数组中
  if (result.data.trainMoney != '0') {
    xChartList.value.push('火车'as never);
    yChartList.value.push(result.data.trainMoney as never);
  }
  if (result.data.flightMoney != '0') {
    xChartList.value.push('飞机'as never);
    yChartList.value.push(result.data.flightMoney as never);
  }
  if (result.data.hotelMoney != '0') {
    xChartList.value.push('酒店' as never);
    yChartList.value.push(result.data.hotelMoney as never);
  }
  init();
};
...
const numPercentage = (num: any) => {
  ...
  let countN: any = countNum.moneyTotal;
