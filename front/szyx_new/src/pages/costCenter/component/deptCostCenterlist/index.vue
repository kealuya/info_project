<template>
  ...
  <van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="min-height:80vh">
    ...
    <div v-if="loading==false && list.length==0" style="height: 500px">
      ...
      <div style="text-align:center;color: rgba(23,26,29,0.40)">暂无成本中心</div>
...
import {getBudgetDeptList} from "../../../../services/budget";
...
const getBudget=async(payload:any,code:any)=>{
  let result: any = await getBudgetDeptList(payload);
  if(code=='refresh'){
    list.value = [];
  }
...
defineExpose({
  getBudget,onRefresh,payload,refresh
})
onMounted(() => {
  onRefresh('1');
});
