  ...
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>会议时间</div>
          <van-field
              v-model="outWorkDate"
              name="openCardDate"
              placeholder="请选择会议时间"
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>会议地点</div>
          <van-field
              v-model="workPlace"
              name="workPlace"
              placeholder="请输入会议地点"
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>会议人员</div>
          <van-field
              v-model="officeStaff"
              name="officeStaff"
              placeholder="请输入会议人员"
              required
              :rules="[{ required: true, message: '请输入会议人员' }]"
          />
        </div>
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>会议内容</div>
          <van-field
...
import {createConferenceRecords,} from "../../../../services/apply";
...
import SelectPeronPicker from '../../../../components/selectComponent/selectPersonPicker/index.vue' // 自定义组件引入需要大写名称（此处用于选择审批人）
import SelectCopyPeoplePicker from '../../../../components/selectComponent/selectCopyPeoplePicker/index.vue' // 此处用于选择抄送人
...
let minDate = new Date(nowDate.getFullYear(), nowDate.getMonth()-6, nowDate.getDay()); // 取六个月前的今天
...
const selectkey = ref();
let selectPersonList= []; // 用于选择审批人列表
let selectCopyPersonList=ref([]); // 用于选择抄送人列表
...
const workPlace=ref(''); //地点
const officeStaff=ref(''); // 会议人员
const meetingContent=ref('') // 会议内容
// 选择审批人
...
const onSubmit = async (values: any) => {
  ...
  const meetingDetail: any = {
    ...
    "meetingEndTime":endTime.value,
    "causeStatement": approveReason.value,
    "meetingDate": [`${startTime.value} 00:00:00`, `${endTime.value} 00:00:00`],
    "meetingPlace":workPlace.value,
    "meetingContent":meetingContent.value,
    "participants": officeStaff.value,
  ...
  const rowShowJson: any = [
    ...
    {'label': '会议地点', 'value': workPlace.value, 'isImportant': '1'},
    {'label': '参会人员', 'value': officeStaff.value, 'isImportant': '1'},
    {'label': '会议内容', 'value': meetingContent.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
  ...
  loading.value=true;
  let result: any = await createConferenceRecords(postData);
