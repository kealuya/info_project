<template>
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span> 请假类型</div>
          <van-field
              v-model="leaveType"
              name="leaveType"
              placeholder="请选择请假类型"
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>请假天数 (天)</div>
          <van-field
              v-model="leaveDate"
              name="leaveDate"
              required
          ...
          >
            <template #input>
              <van-stepper v-model="leaveDays" min="0" :max="leaveSize" step="0.5" readonly/>
            </template>
          </van-field>
        </div>
        <div class="formCard">
        ...
        <div class="formCard">
          <div class="pl5 f16 ml10">
            附件
            <span class="f12 graytxt">最多支持5张附件</span>
          </div>
        ...
        <div class="formCard">
          <div class="pl5 f16">
            <van-row align="center">
              <van-col span="21">
                <span class="require">*</span>审批流程
                <span class="f12 graytxt" v-if="approveList.length!=0">请选择自己组内的负责人进行审批</span>
  ...
  <van-popup v-model:show="selectCopyPeoplePicker" style="width:100vw;height:100vh" position="bottom">
    <select-copy-people-picker @select-over="selectCC" ref="selectCCOver"    :list="selectPersonList"/>
  </van-popup>
</template>

<script lang="ts" setup>
import {ref, inject, onMounted} from "vue";
import {createLeaveApply} from "../../../../services/apply";
...
const leaveType = ref(''); // 请假类型
const leaveStartDate = ref('');// 请假开始时间
const leaveEndDate = ref(''); //请假结束时间
const leaveDate = ref(''); //请假时间展示
const leaveDays = ref(''); //请假天数
const approveReason = ref(''); //申请事由
const flowId = ref(''); //申请事由
const flowName = ref(''); //申请事由
const leaveSize = ref(0); //限制最大天数
...
let selectPersonList= []; // 选择的审批人
let selectCopyPersonList=[]; // 选择的抄送人
let userInfoData: any = inject("userInfo"); //获取审批数组
const selectkey = ref(); // 用于存放选人的用户数组
...
const onConfirm = (value: any) => {
  leaveType.value = value.selectedValues[0];
...
const onConfirmDate = (values: any) => {
  ...
  leaveDays.value = ((end - start) / (1000 * 3600 * 24)).toString();
  leaveSize.value = (end - start) / (1000 * 3600 * 24);
  leaveStartDate.value = moment(start).format("yyyy-MM-DD");
  leaveEndDate.value = moment(end).format("yyyy-MM-DD");
  leaveDate.value = `${leaveStartDate.value} - ${leaveEndDate.value}`;
};
const onSubmit = async (values: any)=> {
  const  leaveDetail:any  = {
    ...
    "deptId": userInfoData.userInfo.deptId,
    "leaveDays": leaveDays.value.toString(),
  ...
  const rowShowJson:any=[
    ...
    {'label':'请假类型','value':leaveType.value,'isImportant':'1'},
    {'label':'开始日期','value':leaveStartDate.value,'isImportant':'1'},
    {'label':'结束日期','value':leaveEndDate.value,'isImportant':'1'},
    {'label':'休假天数','value':leaveDays.value,'isImportant':'1'},
  ];
  const postData:any={
    'approveNo':orderNumberM(),
    ...
    "dateRange": [leaveStartDate.value, leaveEndDate.value],
    "approveType": "请假申请",
  ...
  loading.value=true;
  let result: any = await createLeaveApply(postData);
