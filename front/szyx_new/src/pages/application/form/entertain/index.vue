...
<!--    </van-sticky>-->
    ...
    <div>
      <van-form @submit="onSubmit">
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>宴请时间</div>
          <van-field
              v-model="outWorkDate"
              name="openCardDate"
              placeholder="请选择宴请时间"
              ...
              readonly
              :rules="[{ required: true, message: '请选择宴请时间' }]"
          />
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>宴请地点</div>
          <van-field
              v-model="workPlace"
              name="workPlace"
              placeholder="请输入宴请地点"
              required
              :rules="[{ required: true, message: '请输入宴请地点' }]"
          />
        </div>
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>客户宴请人员</div>
          <van-field
              v-model="officeStaff"
              name="officeStaff"
              placeholder="请输入客户宴请人员"
              required
              :rules="[{ required: true, message: '请输入客户宴请人员' }]"
          />
        </div>
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>陪同人员</div>
          <van-field
...
import {createCustomercBanquet,} from "../../../../services/apply";
...
import SelectPeronPicker from '../../../../components/selectComponent/selectPersonPicker/index.vue' // 自定义组件引入需要大写名称(选择审批人)
import SelectPeoplePicker from '../../../../components/selectComponent/selectPeoplePicker/index.vue' // 选择同行人员组件
import SelectCopyPeoplePicker from '../../../../components/selectComponent/selectCopyPeoplePicker/index.vue' // 选择抄送人员组件
import NavBar from '../../../../components/navBar/index.vue'; // 导航组件
...
const selectPeoplePicker = ref(false); // 控制选择陪同人员
const selectCopyPeoplePicker =ref(false); // 控制选择抄送人员
...
const selectPeopleOver =ref();
const selectkey = ref();
let selectPersonList= []; // 用于选择自定义审批流中的选择审批人
let selectCopyPersonList=ref([]); // 用于选择抄送人
...
const workPlace=ref(''); //宴请地点
const officeStaff=ref(''); // 宴请人员
const accompanyList=ref([]); // 陪同人员
const companion=ref([]); //陪同人员提交数组
const companionTitle=ref(''); //陪同人员提交展示
...
const selectPeople = (list: any) => {
  ...
  list.forEach((item:any)=>{
    ...
    return toSelectCCUserList.push({'empCode':item.empCode,'empName':item.empName});
  });
...
const onSubmit = async (values: any) => {
  ...
  const customercBanquetDetail: any = {
    ...
    "deptId": userInfoData.userInfo.deptId,
    "flowId": flowId.value,
    "occurrenceTime": `${startTime.value}至${endTime.value}`,
    "occurrenceTime1": [`${startTime.value} 00:00:00`,  `${endTime.value} 00:00:00`],
    "banquetObject": officeStaff.value,
    "banquetLocation": workPlace.value,
    'numberOfParticipants':'',
    "causeStatement": approveReason.value,
    'companion':JSON.stringify(companion.value),
    "occurrenceStartTime": `${startTime.value} 00:00:00`,
    "occurrenceEndTime": `${endTime.value} 00:00:00`
  };
  const rowShowJson: any = [
    ...
    {'label': '开始时间', 'value': startTime.value, 'isImportant': '1'},
    {'label': '结束时间', 'value': endTime.value, 'isImportant': '1'},
    {'label': '宴请地点', 'value': workPlace.value, 'isImportant': '1'},
    {'label': '宴请人员', 'value': officeStaff.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
    ...
    "timeStatus": false,
    "approveType": "客户宴请",
    "customercBanquetDetail": customercBanquetDetail,
  ...
  loading.value=true;
  let result: any = await createCustomercBanquet(postData);
