<template>
  <div>
    ...
    <div>
      <van-form @submit="onSubmit">
        ...
        <div class="formCard">
          <div class="pl5 f16"><span class="require">*</span>签批备注</div>
          <van-field
  ...
  <van-popup v-model:show="selectCopyPeoplePicker" style="width:100vw;height:100vh" position="bottom">
    <select-copy-person-picker @select-over="selectCC" ref="selectCcOver"   :list="selectCopyPersonList"/>
  </van-popup>
</template>

<script lang="ts" setup>
import {ref, inject, onMounted} from "vue";
import {createShareholderSign,} from "../../../../services/apply";
...
import SelectCopyPersonPicker from '../../../../components/selectComponent/selectCopyPeoplePicker/index.vue' //选择抄送人组件
...
const selectCopyPeoplePicker=ref(false); // 控制选择抄送人的组件
...
const selectkey = ref();
let selectPersonList= []; // 选中的审批人员
let selectCopyPersonList=[]; // 选中的抄送人员
...
const onSubmit = async (values: any) => {
  ...
  const rowShowJson: any = [
    ...
    {'label': '申请事由', 'value': reasons.value, 'isImportant': '1'},
    {'label': '签批备注', 'value': approveReason.value, 'isImportant': '1'},
  ];
  const postData: any = {
    'approveNo':ywNo.value,
    ...
    "timeStatus": false,
    "approveType": "董事签批",
    "shareholderSign": shareholderSign,
  ...
  let result: any = await createShareholderSign(postData);
  loading.value=true;
