...
<!--        </div>-->
<!--      </div>-->
<!--&lt;!&ndash;  <div class="footer_btn">&ndash;&gt;-->
<!--&lt;!&ndash;    <van-button block type="primary" @click="taskProcessingHandle">&ndash;&gt;-->
<!--&lt;!&ndash;      去完成&ndash;&gt;-->
...
import bgc from '../../assets/img/task_bgc_02.png'
import { ref } from 'vue';
...
const taskTitle = ref<string>('生物标志物作为高血压患者风险评估的有效指标')
const picSrc = ref<string>(bgc)
const target = ref<string>('间隙保持器有助于维持乳牙缺失后的间隙空间安装间隙保持架后，间隙可以保持在一定程度上。恒磨牙萌出后，取下间隙固位体，最大限度地保持牙齿的咬合平衡。')
const content = ref<string>('探讨聚醚酮酮(PEKK)数字化乳牙早失间隙保持器的临床应用效果。方法从2019年4—5月在南京大学医学院附属口腔医院儿童口腔科就诊的6~10岁儿童中选取下颌单侧乳磨牙早失患者共5例,其中男2例、女3例,经数字化口内光学印模制取,间隙保持器数字化设计,PEKK基材间隙保持器的数字化制作,口内试戴调整,粘接。结果5例患儿的数字化乳牙早失间隙保持器重量只有传统金属带环丝圈保持器的25%,设计和制作时间只需1 h,5例数字化间隙保持器口内观察与基牙吻合度高。结论数字化乳牙早失间隙保持器采用数字化口内印模法比传统藻酸盐印模法提高了患儿的舒适度,可当日设计完成并佩戴,并使用PEKK材料,实现了间隙保持器去金属化,无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景。无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景无金属过敏等风险,兼顾了美观的效果,具有较好的临床应用前景\n')
const onClickLeft = () => {
  // history.go(-2)
