...
<template>
  <div class="selectPersonPicker">
    <van-sticky>
      <navbar title="选择申请单" left-arrow left-text="返回" @leftEvent="$emit('selectOver',[])" />
      <form action="/">
    ...
    <div>
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="background:#f1f2f3; min-height: 100vh;">
        ...
        <div v-if="loading==false && list.length==0" style="height: 500px">
          ...
          <div style="color: rgba(23,26,29,0.40);text-align: center">暂无可以用的成本中心</div>
        ...
        >
          <div class="ticket_content" v-for="(item,index) in list" :key="item">
            <apply-card :get-data="item"  @click="costCenterOK(item)" :check-key="costCostData" />
          </div>
        </van-list>
      </van-pull-refresh>
    </div>
...
const getCostCenterList=async(payload:any,code:any)=>{
  let result: any = await getApplyList(payload);
  if(code=='refresh'){
    list.value = [];
  }
