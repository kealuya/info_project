...
func RecoverHandler(f func(err error)) {
	if err := recover(); err != nil {
		...
		if configs.SENTRY_FLAG {
			sentry.CaptureException(errors.New(fmt.Sprintf("★★★错误error: %s", err)))
...
func ErrorHandler(err error, info ...interface{}) {
	if err != nil {
		if configs.SENTRY_FLAG {
			sentry.CaptureException(errors.New(fmt.Sprintf("★★★错误error: %s", err.Error())))
			defer sentry.Flush(2 * time.Second)
		}
...
//共通logs  info处理方法 发送给sentry 记录log
func SentryInfoHandler(f string) {
	if configs.SENTRY_FLAG {
