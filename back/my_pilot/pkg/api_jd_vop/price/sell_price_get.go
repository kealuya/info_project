...
func GetSellPrice(sku []string, skuInfos []struct{ int64 int }, queryExts string) (*api_jd_vop.Response[[]SellPriceResult], error) {

	config := api_jd_vop.GetJdIopConfig()
	url := config["jd_vop"]["base_url"] + "price/getSellPrice"
	...
	if skuInfos != nil {
		marshal, err := api_jd_vop.JsonEncoder.Marshal(skuInfos)
		if err != nil {
			return nil, err
		}
	...
	resultResp := &api_jd_vop.Response[[]SellPriceResult]{}
